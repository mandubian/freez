...
object TFingerTree {
  ...
  def deep[R[_, _], A, B, C, D](
    ...
    // lazy val prefixz = prefix
    // lazy val middlez = middle
    // lazy val suffixz = suffix
    ...
    tree.suffix match {
      ...
      case _ =>
        Deep(
          tree.prefix,
          tree.middle,
          Digit.appendd(tree.suffix, One(a))
        )
    }
  }
  ...
  def deepL[R[_, _], A, B, C, D](
    ...
    pr match {
      case _:ZNil[R, A] =>
        TFingerTreeSeq.tviewl[TNode, B, C](m) match {
          case EmptyL() =>
